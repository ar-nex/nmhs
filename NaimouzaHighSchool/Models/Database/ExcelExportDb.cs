...
namespace NaimouzaHighSchool.Models.Database
{
    public class ExcelExportDb : BaseDb
    {
        ...
        public List<Student> GetStudentListByClass(string cls, int sYear, int eYear)
        {
            ...
            string sql = $"SELECT s.*, c.*, a.* FROM {mainTable} s INNER JOIN student_class c ON c.student_basic_id = s.id INNER JOIN admission a ON a.student_basic_id = s.id WHERE c.class = '{cls}' AND c.startYear = '{sYear.ToString()}' AND c.endYear = '{eYear.ToString()}'";
            try
            {
                this.conn.Open();
        ...
        public List<Student> GetStudentListByClass(string cls, string sec, int sYear, int eYear)
        {
            ...
            string sql = $"SELECT s.*, c.*, a.* FROM {mainTable} s INNER JOIN student_class c ON c.student_basic_id = s.id INNER JOIN admission a ON a.student_basic_id = s.id WHERE c.class = '{cls}' AND c.section = '{sec}' AND c.startYear = '{sYear.ToString()}' AND c.endYear = '{eYear.ToString()}'";
            try
            {
                this.conn.Open();
                MySqlCommand cmd = new MySqlCommand(sql, this.conn);
        ...
        private List<Student> BuildStudentList(MySqlDataReader rdr)
        {
            List<Student> slist = new List<Student>();
            while (rdr.Read())
            {
